...
<form action="admin/location/editdistrict/{{$data['id']}}" method="POST" enctype="multipart/form-data">
						...
						<div class="control-group">
							<label>Level</label>
						  	<select name="city_id" class="form-control">
						  		@foreach($city as $val)
						  		<option <?php if($data['city_id'] == $val['id']){echo "selected";} ?> value="{{$val->id}}">{{$val->name}}</option>
						  		@endforeach
						  	</select>
					  	</div>
					  	<div class="control-group">
